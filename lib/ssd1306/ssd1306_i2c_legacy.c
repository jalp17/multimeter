...
void i2c_master_init(SSD1306_t * dev, int16_t sda, int16_t scl, int16_t reset)
{
	...
	// Enable internal pull-up resistors explicitly on SDA and SCL pins
...
#ifndef OLED_CMD_SET_IREF_SELECT
#define OLED_CMD_SET_IREF_SELECT 0xAD
#endif
...
void i2c_init(SSD1306_t * dev, int width, int height) {
	...
	if (dev->_width > 2 * dev->_height) i2c_master_write_byte(cmd, 0x02, true);
	else i2c_master_write_byte(cmd, 0x12, true);
	...
	i2c_master_write_byte(cmd, OLED_CMD_SET_IREF_SELECT, true);			// AD
	i2c_master_write_byte(cmd, 0x30, true);
	i2c_master_write_byte(cmd, OLED_CMD_SET_PRECHARGE, true);			// D9
	// No _external_vcc field, so use false (no external VCC)
